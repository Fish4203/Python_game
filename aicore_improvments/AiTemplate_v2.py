...
class AI():
    ...
    def __init__(self,n,outputlayers):
        ...
        self.weightout = np.array([random.uniform(1,-1) for x in range(self.outputlayers*self.n)],dtype='float32').reshape(self.outputlayers, self.n)

        # bias's
        self.bias1 = np.array([random.uniform(5,-5) for y in range(self.n)],dtype='float32')
        self.bias2 = np.array([random.uniform(5,-5) for y in range(self.n)],dtype='float32')
    ...
    def train(self,distance):
        ...
        for i in range(self.n):
            self.bias1[i] += random.uniform(-distance,distance)
    ...
    def exportAI(self):
        ...
        pickle.dump(self.bias1, open('bias1.p', 'wb'))
    ...
    def importAI(self):
        ...
        self.bias1 = pickle.load(open('bias1.p', 'rb'))
